...
varSubject="Intransit Report On -"+dateDay+' '+monthName+' '+dateYear;
	}else if(argMailType ==9 ){
		varSubject="Collection vs Deposit reconciliation YTD -"+dateDay+' '+monthName+' '+dateYear;
	}else if(argMailType ==10 ){
		varSubject="Collection vs Deposit reconciliation YTD -"+dateDay+' '+monthName+' '+dateYear;
	}
	else{
...
exports.cashReconCHEmail=function(argTemplate,emailres,fiCHEmailres,argDate,argMailType,callback){
	...
	service: 'gmail',
	auth: {
	    user: fiCHEmailres[0].fromid,
	   pass: fiCHEmailres[0].passcode
	}
	});
	let mailOptions={
	  from: fiCHEmailres[0].fromid,
	  to:emailres[0].ch_email,
	  bcc:fiCHEmailres[0].bccid,
	  cc:emailres[0].sch_email,
	  subject: varSubject,
	  html: argTemplate
	};
