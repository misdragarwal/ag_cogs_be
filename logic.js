...
let overseasCurrencyConversion = async (ftddate,currencyres,currencylastres) => {
	...
	let overseasftd=0,overseasmtd=0,mozamftd=0,mozammtd=0,overseastempObj={},mauritiusftd=0,mauritiusmtd=0,zambiaftd=0,zambiamtd=0;
...
//console.log(currencyres);
	...
	let overseasCountryCode = ['MDR','NGA','RWD','ZMW','GHA','NAB','UGD','TZA'];
	...
	if((currencyres.length>0) && (ftddate>='2019-11-01')){
		...
		if((Object.keys(overseastempObj).length) > 0){		
			...
			if(mozamftd==0){	
				...
				let zambiaarr = _.filter(currencyres, { country_code: 'ZMW' })
				_.filter(zambiaarr, { currency_date: ftddate }).forEach(element => {
					zambiaftd += element.INR_rate
				})
				
	
				if(zambiaftd==0){	
					_.filter(currencylastres, { country_code: 'ZMW', currency_date: lastdate }).forEach(element => {
							zambiaftd = element.INR_rate;
							zambiamtd = element.INR_rate;
							
					})
				}else{				
					zambiaarr.forEach(element => {
					zambiamtd += element.INR_rate;				
					})
					zambiamtd = zambiamtd/(zambiaarr.length);
				}
			...
			overseastempObj['ZMB']= {'ftd' : zambiaftd,'mtd' :zambiamtd};
			overseastempObj['KWE']= {'ftd' : zambiaftd,'mtd' :zambiamtd};
			overseastempObj['ZMW']= {'ftd' : zambiaftd,'mtd' :zambiamtd};
...
let filterEntity = async (dbres, dbres2, ftddate,overseasCurrency) => {
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['ftdpha']*overseasCurrency.ZMW.ftd);
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['ftdopt']*overseasCurrency.ZMW.ftd);
	}
	 ...
	 if((Object.keys(overseasCurrency).length) > 0){
		 ...
		 (tempObj.OZA['ftdlab']*overseasCurrency.ZMW.ftd);
	 }
	...
	ohcftdot = tempObj.OGH['ftdot'] + tempObj.OMA['ftdot'] + tempObj.OMD['ftdot'] + tempObj.OMZ['ftdot'] + tempObj.ORB['ftdot'] + tempObj.ONA['ftdot'] + tempObj.ORW['ftdot'] +  tempObj.OTA['ftdot'] +  tempObj.OUG['ftdot'] + tempObj.OZA['ftdot']+ tempObj.ONI['ftdot']; 
	 ...
	 if((Object.keys(overseasCurrency).length) > 0){
		 ...
		 (tempObj.OZA['ftdot']*overseasCurrency.ZMW.ftd); 
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['ftd']*overseasCurrency.ZMW.ftd);
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['ftdrev']*overseasCurrency.ZMW.ftd);
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtdpha']*overseasCurrency.ZMW.mtd) ; 
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtdopt']*overseasCurrency.ZMW.mtd);
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtdlab']*overseasCurrency.ZMW.mtd);
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtdot']*overseasCurrency.ZMW.mtd); 
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtd']*overseasCurrency.ZMW.mtd); 
	}
	...
	if((Object.keys(overseasCurrency).length) > 0){
		...
		(tempObj.OZA['mtdrev']*overseasCurrency.ZMW.mtd);
	}
...
let filterGroupwise = async (aeh, ahc,ohc, dbres2, branches, ftddate, vobres, breakupres, breakupmtdres, overseasCurrency) => {
    ...
    let ahcGroups = ['Chennai branches', 'Pondycherry','Tirunelveli', 'Coimbatore','Tuticorin + Madurai', 'Trichy', 'Thanjavur','Tiruppur' ,'Port Blair', 'Karnataka', 'Banglore', 'Hubli + Mysore', 'Maharashtra', "Mumbai",
	"Rest of Maharashtra",'Telangana', 'Hyderabad', 'Andhra Pradesh', 'Rest of India(incl. Jaipur)', 'Kerala', 'Kolkata', 'Ahmedabad', 'Madhya Pradesh', 'Odisha']
    let aehgroupedBranches = {
        ...
        'Kum + Ney + Vil': ["KBK", "NVL", "VPM","KCH"],
        'Dha + Salem + Krish': ["DHA", "SLM", "KSN"],
        'Erode + Hosur': ["ERD", "HSR"],
        'Jaipur': ["JPR"],
		"20|20 Marvel Centers": ["GOW","ABR","VLY","SKL"],
        'Madurai KK Nagar': ["MDU"]
    }
    let ahcgroupedBranches = {
        'Chennai branches': ["TBM", "ADY", "EGM", "MGP", "NWP", "AMB","TVT","CMP"],
        'Pondycherry': ["PDY"],
        'Tirunelveli': ["TVL"],
		'Coimbatore': ["CMB","LMS"],
		...
		'Maharashtra' :["VSH", "PUN", "HDP","CMR","NMH","CGR","MMH","KTD","PCH","SNG","VNR","VWD"], 
		"Mumbai" :["VSH","CMR"], 
		"Rest of Maharashtra" :["PUN","HDP","NMH","CGR","MMH","KTD","PCH","SNG","VNR","VWD"], 
        'Telangana': ["DNR", "HMH", "MDA", "SNR", "HIM", "SBD","MPM","GCB"],
        'Hyderabad': ["DNR", "HMH", "MDA", "SNR", "HIM", "SBD","MPM","GCB"],
        'Andhra Pradesh': ["VMH", "NEL", "GUN", "TPT", "RAJ","NKR","OTR","MPE"],
...
function mtdGRpre(currentyear,lastpreyear){
	let monthGD = 0;
	let monthGDpre = 0;
	monthGD = currentyear / lastpreyear;
	let timeperoid = 1/2;
	
	
	monthGDpre = (((Math.pow(monthGD, timeperoid))-1)*100).toFixed(1);
	
	if(currentyear<=0 || isNaN(monthGDpre)==true || monthGDpre=='Infinity' || monthGDpre=='-Infinity'){
		return '-';
	}else{
		return monthGDpre;
	}
...
let oprPer = (currentyear, lastyear) => {
    if (currentyear> 0 ) {
        return ((currentyear / lastyear) * 100).toFixed(1);
    }
    else{
		return '-';
	}
...
exports.snapshotR = async ( 
  ...
  opr_terget_query_res
	...
	let reveCalMonthRes = await reveCalMonthWise( rev_det_res,rev_target_res,threeyearsbef,twoyearsbef,currentFinYear,currentFinYearTo,opd_det_res,opr_res,opr_terget_query_res);
	let reveCalQRes = await reveCalQWise(reveCalMonthRes);
	...
	let goalText = 'Gr % FY'+currentFinYear.slice(-2)+' vs FY'+currentFinYearTo.slice(-2);
	let goalText1 = 'Gr % FY'+String(twoyearsbef).slice(-2)+' vs FY'+currentFinYearTo.slice(-2);
...
let reveCalMonthWise = async ( rev_det_res,rev_target_res,threeyearsbef,twoyearsbef,currentFinYear,currentFinYearTo,opd_det_res,opr_res,opr_terget_query_res) => {
	...
	let allcatmtd=0,allcatcount=0,allcatmtdtotal=0,allcatcounttotal=0;
   ...
   curFinYearMonthSet[currentFinYearTo] = [1,2,3];
	...
	prevFinYearArr.forEach(year => {		     
			 prevFinYearMonthSet[year].forEach(month => {
				...
				_.filter(opr_res, {   'month': month,'year' : year}).forEach(
					element => {
						
						
						
						ctlowend+= element.cat_low_end_value;
						ctlowendcount+= element.cat_low_end_no;
						ctmidend+= element.cat_mid_end_value;
						ctmidendcount+= element.cat_mid_end_no;
						cthighend+= element.cat_high_end_value;
						cthighendcount+= element.cat_high_end_no;
						allcatmtd += element.cat_surg_value;
						allcatcount+= element.cat_surg_no;
						mtdopt+= element.opt_value;
						mtdoptcount+= element.opt_num_orders;
						mtdoptypp+=element.optical_ypp;
						mtdpha+= element.pha_value;
						mtdphacount+= element.pha_no_bills;
						mtdphaypp+= element.pha_ypp;
						mtdtreat+= element.treat_value;
						mtdtreatcount+= element.treat_no;
						mtdinv+= element.inv_value;
						mtdinvcount+= element.inv_no;
						mtdcor+= element.corneal_value;
						mtdcorcount+= element.corneal_no;
						mtdref+= element.ref_value;
						mtdrefcount+= element.ref_no;
						mtdosu+= element.other_sug_value;
						mtdosucount+= element.other_sug_no;
						mtdconlens+= element.con_len_value;
						mtdvrin+= element.vr_inje_value;
						mtdvrincount+= element.vr_inje_no;
						mtdtconsu+= element.totl_consu_charges;						
						mtdpopdcount+= element.paid_review;
						mtdvrsug+= element.vr_surg_value;
						mtdvrsugcount+= element.vr_surg_no;
				...
				allcatmtdtotal+= parseFloat(allcatmtd);
				allcatcounttotal+= parseInt(allcatcount);
	...
	branchObj['lastyeartotal'] = {
					...
					totalallcat: lakshFormatRevenue1(allcatmtdtotal),
					totalallcatcount: allcatcounttotal,
    ...
    allcatmtd=0,allcatcount=0,allcatmtdtotal=0,allcatcounttotal=0;
	opdtarget=0,paidrevtarget=0,conschargestarget=0,catlenostarget=0,catmenostarget=0,cathenostarget=0,catlevaluestarget=0,catmevaluestarget=0,cathevaluestarget=0;
	...
	branchObj['totalper'] = { mtdrevper : oprPer(parseFloat(branchObj.currentyeartotal.totalrev),parseFloat(branchObj.lastyeartotal.totalrev)),
							mtdrevperthird : mtdGRpre(branchObj.currentyeartotal.totalrev,branchObj.thirdyeartotal.totalrev),
							
							
							
							mtdtargetper : oprPer(parseFloat(branchObj.currentyeartotal.totaltarget),parseFloat(branchObj.lastyeartotal.totaltarget)),
							mtdtargetperthird : mtdGRpre(branchObj.currentyeartotal.totaltarget,branchObj.thirdyeartotal.totaltarget),							
							
							mtdopdper : oprPer(parseFloat(branchObj.currentyeartotal.totalopd),parseFloat(branchObj.lastyeartotal.totalopd)),
							mtdopdperthird : mtdGRpre(branchObj.currentyeartotal.totalopd,branchObj.thirdyeartotal.totalopd),
							
							opdyppper : oprPer(parseFloat(branchObj.currentyeartotal.totalopdypp),parseFloat(branchObj.lastyeartotal.totalopdypp)),
							opdyppperthird : mtdGRpre(branchObj.currentyeartotal.totalopdypp,branchObj.thirdyeartotal.totalopdypp),
							
							ctlowendper : oprPer(parseFloat(branchObj.currentyeartotal.totalctlowend),parseFloat(branchObj.lastyeartotal.totalctlowend)),
							ctlowendperthird : mtdGRpre(branchObj.currentyeartotal.totalctlowend,branchObj.thirdyeartotal.totalctlowend),
							
							
							cthighendper : oprPer(parseFloat(branchObj.currentyeartotal.totalcthighend),parseFloat(branchObj.lastyeartotal.totalcthighend)),
							cthighendperthird : mtdGRpre(branchObj.currentyeartotal.totalcthighend,branchObj.thirdyeartotal.totalcthighend),
							
							
							ctmidendper : oprPer(parseFloat(branchObj.currentyeartotal.totalctmidend),parseFloat(branchObj.lastyeartotal.totalctmidend)),
							ctmidendperthird : mtdGRpre(branchObj.currentyeartotal.totalctmidend,branchObj.thirdyeartotal.totalctmidend),
							
							
							ctlowendcountper : oprPer(parseFloat(branchObj.currentyeartotal.totalctlowendcount),parseFloat(branchObj.lastyeartotal.totalctlowendcount)),
							ctlowendcountperthird : mtdGRpre(branchObj.currentyeartotal.totalctlowendcount,branchObj.thirdyeartotal.totalctlowendcount),
							
							cthighendcountper : oprPer(parseFloat(branchObj.currentyeartotal.totalcthighendcount),parseFloat(branchObj.lastyeartotal.totalcthighendcount)),
							cthighendcountperthird : mtdGRpre(branchObj.currentyeartotal.totalcthighendcount,branchObj.thirdyeartotal.totalcthighendcount),
							
							
							ctmidendcountper:oprPer(parseFloat(branchObj.currentyeartotal.totalctmidendcount),parseFloat(branchObj.lastyeartotal.totalctmidendcount)),
							ctmidendcountperthird:mtdGRpre(branchObj.currentyeartotal.totalctmidendcount,branchObj.thirdyeartotal.totalctmidendcount),
							
							
							ctallper : oprPer(parseFloat(branchObj.currentyeartotal.totalallcat),parseFloat(branchObj.lastyeartotal.totalallcat)),
							ctallperthird : mtdGRpre(branchObj.currentyeartotal.totalallcat,branchObj.thirdyeartotal.totalallcat),
							
											
							
							ctallcountper:oprPer(parseFloat(branchObj.currentyeartotal.totalallcatcount),parseFloat(branchObj.lastyeartotal.totalallcatcount)),
							ctallcountperthird:mtdGRpre(branchObj.currentyeartotal.totalallcatcount,branchObj.thirdyeartotal.totalallcatcount),
							
							optper : oprPer(parseFloat(branchObj.currentyeartotal.totalopt),parseFloat(branchObj.lastyeartotal.totalopt)),
							optperthird : mtdGRpre(branchObj.currentyeartotal.totalopt,branchObj.thirdyeartotal.totalopt),
							
							
							optcountper:oprPer(parseFloat(branchObj.currentyeartotal.totaloptcount),parseFloat(branchObj.lastyeartotal.totaloptcount)),
							optcountperthird:mtdGRpre(branchObj.currentyeartotal.totaloptcount,branchObj.thirdyeartotal.totaloptcount),
							
							
							optyppper : oprPer(parseFloat(branchObj.currentyeartotal.totalyppopt),parseFloat(branchObj.lastyeartotal.totalyppopt)),
							optyppperthird : mtdGRpre(branchObj.currentyeartotal.totalyppopt,branchObj.thirdyeartotal.totalyppopt),
							
							
							
							phaper : oprPer(parseFloat(branchObj.currentyeartotal.totalpha),parseFloat(branchObj.lastyeartotal.totalpha)),
							phaperthird : mtdGRpre(branchObj.currentyeartotal.totalpha,branchObj.thirdyeartotal.totalpha),
							
							
							phacountper:oprPer(parseFloat(branchObj.currentyeartotal.totalphacount),parseFloat(branchObj.lastyeartotal.totalphacount)),
							phacountperthird:mtdGRpre(branchObj.currentyeartotal.totalphacount,branchObj.thirdyeartotal.totalphacount),
							
							
							phayppper : oprPer(parseFloat(branchObj.currentyeartotal.totalypppha),parseFloat(branchObj.lastyeartotal.totalypppha)),
							phayppperthird : mtdGRpre(branchObj.currentyeartotal.totalypppha,branchObj.thirdyeartotal.totalypppha),
							
							treatinvper : oprPer(parseFloat(branchObj.currentyeartotal.totaltreatinv),parseFloat(branchObj.lastyeartotal.totaltreatinv)),
							treatinvperthird : mtdGRpre(branchObj.currentyeartotal.totaltreatinv,branchObj.thirdyeartotal.totaltreatinv),
							
							
							treatinvcountper:oprPer(parseFloat(branchObj.currentyeartotal.totaltreatinvcount),parseFloat(branchObj.lastyeartotal.totaltreatinvcount)),
							treatinvcountperthird:mtdGRpre(branchObj.currentyeartotal.totaltreatinvcount,branchObj.thirdyeartotal.totaltreatinvcount),
							
							
							corper : oprPer(parseFloat(branchObj.currentyeartotal.totalcor),parseFloat(branchObj.lastyeartotal.totalcor)),
							corperthird : mtdGRpre(branchObj.currentyeartotal.totalcor,branchObj.thirdyeartotal.totalcor),
							
							
							corcountper:oprPer(parseFloat(branchObj.currentyeartotal.totalcorcount),parseFloat(branchObj.lastyeartotal.totalcorcount)),
							corcountperthird:mtdGRpre(branchObj.currentyeartotal.totalcorcount,branchObj.thirdyeartotal.totalcorcount),
							
							
							refper : oprPer(parseFloat(branchObj.currentyeartotal.totalref),parseFloat(branchObj.lastyeartotal.totalref)),
							refperthird : mtdGRpre(branchObj.currentyeartotal.totalref,branchObj.thirdyeartotal.totalref),
							
							
							refcountper:oprPer(parseFloat(branchObj.currentyeartotal.totalrefcount),parseFloat(branchObj.lastyeartotal.totalrefcount)),
							refcountperthird:mtdGRpre(branchObj.currentyeartotal.totalrefcount,branchObj.thirdyeartotal.totalrefcount),
							
							osuper : oprPer(parseFloat(branchObj.currentyeartotal.totalosu),parseFloat(branchObj.lastyeartotal.totalosu)),
							osuperthird : mtdGRpre(branchObj.currentyeartotal.totalosu,branchObj.thirdyeartotal.totalosu),
							
							
							osucountper:oprPer(parseFloat(branchObj.currentyeartotal.totalosucount),parseFloat(branchObj.lastyeartotal.totalosucount)),
							osucountperthird:mtdGRpre(branchObj.currentyeartotal.totalosucount,branchObj.thirdyeartotal.totalosucount),
							
							conlensper : oprPer(parseFloat(branchObj.currentyeartotal.totalconlens),parseFloat(branchObj.lastyeartotal.totalconlens)),
							conlensperthird : mtdGRpre(branchObj.currentyeartotal.totalconlens,branchObj.thirdyeartotal.totalconlens),
							
							
							conlenscountper:oprPer(parseFloat(branchObj.currentyeartotal.totalconlenscount),parseFloat(branchObj.lastyeartotal.totalconlenscount)),
							conlenscountperthird:mtdGRpre(branchObj.currentyeartotal.totalconlenscount,branchObj.thirdyeartotal.totalconlenscount),
							
							vrinper : oprPer(parseFloat(branchObj.currentyeartotal.totalvrin),parseFloat(branchObj.lastyeartotal.totalvrin)),
							vrinperthird : mtdGRpre(branchObj.currentyeartotal.totalvrin,branchObj.thirdyeartotal.totalvrin),
							
							
							vrincountper:oprPer(parseFloat(branchObj.currentyeartotal.totalvrincount),parseFloat(branchObj.lastyeartotal.totalvrincount)),
							vrincountperthird:mtdGRpre(branchObj.currentyeartotal.totalvrincount,branchObj.thirdyeartotal.totalvrincount),
							
							tconsuper : oprPer(parseFloat(branchObj.currentyeartotal.totaltconsu),parseFloat(branchObj.lastyeartotal.totaltconsu)),
							tconsuperthird : mtdGRpre(branchObj.currentyeartotal.totaltconsu,branchObj.thirdyeartotal.totaltconsu),
							
							popdper : oprPer(parseFloat(branchObj.currentyeartotal.totalpopdcount),parseFloat(branchObj.lastyeartotal.totalpopdcount)),
							popdperthird : mtdGRpre(branchObj.currentyeartotal.totalpopdcount,branchObj.thirdyeartotal.totalpopdcount),
							
							vrsugper : oprPer(parseFloat(branchObj.currentyeartotal.totalvrsug),parseFloat(branchObj.lastyeartotal.totalvrsug)),
							vrsugperthird : mtdGRpre(branchObj.currentyeartotal.totalvrsug,branchObj.thirdyeartotal.totalvrsug),
							
							
							vrsugcountper:oprPer(parseFloat(branchObj.currentyeartotal.totalvrsugcount),parseFloat(branchObj.lastyeartotal.totalvrsugcount)),
							vrsugcountper:mtdGRpre(branchObj.currentyeartotal.totalvrsugcount,branchObj.thirdyeartotal.totalvrsugcount)
	...
	branchObj.lastyearmonth.forEach(key => {
		...
		monthWiseObj.push({			
					...
					mtdrevper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrev),parseFloat(key.mtdrev)),
					mtdrevperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdrev,branchObj.thirdyearmonth[k].mtdrev),
					
					
					mtdtargetper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrevtarget),parseFloat(key.mtdrevtarget)),					
					mtdtargetperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdrevtarget,branchObj.thirdyearmonth[k].mtdrevtarget),
					
					
					mtdopdper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdopd),parseFloat(key.mtdopd)),
					mtdopdperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdopd,branchObj.thirdyearmonth[k].mtdopd),
					
					mtdopdyppper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdopdypp),parseFloat(key.mtdopdypp)),
					mtdopdyppperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdopdypp,branchObj.thirdyearmonth[k].mtdopdypp),
					
					mtdctlowendper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdctlowend),parseFloat(key.mtdctlowend)),
					mtdctlowendperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdctlowend,branchObj.thirdyearmonth[k].mtdctlowend),
					
					
					mtdcthighendper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdcthighend),parseFloat(key.mtdcthighend)),
					mtdcthighendperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdcthighend,branchObj.thirdyearmonth[k].mtdcthighend),
					
					
					mtdctmidendper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdctmidend),parseFloat(key.mtdctmidend)),
					mtdctmidendperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdctmidend,branchObj.thirdyearmonth[k].mtdctmidend),
					
					mtdctlowendcountper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdctlowendcount),parseFloat(key.mtdctlowendcount)),
					mtdctlowendcountperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdctlowendcount,branchObj.thirdyearmonth[k].mtdctlowendcount),
					
					
					mtdcthighendcountper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdcthighendcount),parseFloat(key.mtdcthighendcount)),
					mtdcthighendcountperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdcthighendcount,branchObj.thirdyearmonth[k].mtdcthighendcount),
					
					
					mtdctmidendcountper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdctmidendcount),parseFloat(key.mtdctmidendcount)),
					mtdctmidendcountperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdctmidendcount,branchObj.thirdyearmonth[k].mtdctmidendcount),

					
					mtdallcatper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdallcat),parseFloat(key.mtdallcat)),
					mtdallcatperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdallcat,branchObj.thirdyearmonth[k].mtdallcat),
					
					
					mtdallcatcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdallcatcount),parseFloat(key.mtdallcatcount)),
					mtdallcatcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdallcatcount,branchObj.thirdyearmonth[k].mtdallcatcount),
					
					mtdoptper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdopt),parseFloat(key.mtdopt)),
					mtdoptperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdopt,branchObj.thirdyearmonth[k].mtdopt),
					
					mtdoptcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdoptcount),parseFloat(key.mtdoptcount)),
					mtdoptcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdoptcount,branchObj.thirdyearmonth[k].mtdoptcount),
					
					
					mtdoptyppper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdoptypp),parseFloat(key.mtdoptypp)),
					mtdoptyppperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdoptypp,branchObj.thirdyearmonth[k].mtdoptypp),
					
					
					mtdphaper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdpha),parseFloat(key.mtdpha)),
					mtdphaperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdpha,branchObj.thirdyearmonth[k].mtdpha),
					
					
					mtdphacountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdphacount),parseFloat(key.mtdphacount)),
					mtdphacountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdphacount,branchObj.thirdyearmonth[k].mtdphacount),
					
					
					mtdphayppper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdphaypp),parseFloat(key.mtdphaypp)),
					mtdphayppperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdphaypp,branchObj.thirdyearmonth[k].mtdphaypp),
					
					mtdtreatinvper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdtreatinv),parseFloat(key.mtdtreatinv)),
					mtdtreatinvperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdtreatinv,branchObj.thirdyearmonth[k].mtdtreatinv),
					
					mtdtreatinvcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdtreatinvcount),parseFloat(key.mtdtreatinvcount)),
					mtdtreatinvcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdtreatinvcount,branchObj.thirdyearmonth[k].mtdtreatinvcount),
					
					mtdcorper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdcor),parseFloat(key.mtdcor)),
					mtdcorperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdcor,branchObj.thirdyearmonth[k].mtdcor),
					
					
					mtdcorcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdcorcount),parseFloat(key.mtdcorcount)),
					mtdcorcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdcorcount,branchObj.thirdyearmonth[k].mtdcorcount),
					
					
					mtdrefper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdref),parseFloat(key.mtdref)),
					mtdrefperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdref,branchObj.thirdyearmonth[k].mtdref),
					
					
					mtdrefcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrefcount),parseFloat(key.mtdrefcount)),
					mtdrefcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdrefcount,branchObj.thirdyearmonth[k].mtdrefcount),
					
					mtdosuper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdosu),parseFloat(key.mtdosu)),
					mtdosuperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdosu,branchObj.thirdyearmonth[k].mtdosu),
					
					
					mtdosucountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdosucount),parseFloat(key.mtdosucount)),
					mtdosucountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdosucount,branchObj.thirdyearmonth[k].mtdosucount),
					
					mtdconlensper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdconlens),parseFloat(key.mtdconlens)),
					mtdconlensperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdconlens,branchObj.thirdyearmonth[k].mtdconlens),
					
					mtdconlenscountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdconlenscount),parseFloat(key.mtdconlenscount)),
					mtdconlenscountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdconlenscount,branchObj.thirdyearmonth[k].mtdconlenscount),
					
					mtdvrinper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdvrin),parseFloat(key.mtdvrin)),
					mtdvrinperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdvrin,branchObj.thirdyearmonth[k].mtdvrin),
					
					
					mtdvrincountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdvrincount),parseFloat(key.mtdvrincount)),
					mtdvrincountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdvrincount,branchObj.thirdyearmonth[k].mtdvrincount),
					
					mtdtconsuper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdtconsu),parseFloat(key.mtdtconsu)),
					mtdtconsuperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdtconsu,branchObj.thirdyearmonth[k].mtdtconsu),
					
					
					mtdpopdper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdpopdcount),parseFloat(key.mtdpopdcount)),
					mtdpopdperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdpopdcount,branchObj.thirdyearmonth[k].mtdpopdcount),
					
					mtdvrsugper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdvrsug),parseFloat(key.mtdvrsug)),
					mtdvrsugperythird:mtdGRpre(branchObj.currentyearmonth[k].mtdvrsug,branchObj.thirdyearmonth[k].mtdvrsug),
					
					
					mtdvrsugcountper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdvrsugcount),parseFloat(key.mtdvrsugcount)),
					mtdvrsugcountperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdvrsugcount,branchObj.thirdyearmonth[k].mtdvrsugcount)
...
let reveCalQWise = async ( reveCalMonthRes) => {
	...
	for (let key in QObj.lastyearQ) {
		...
		QRevenueObj3[key].push({			
					mtdrevper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrev),parseFloat(QObj.lastyearQ[key][0].mtdrev)),
					mtdrevperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdrev,QObj.thirdyearQ[key][0].mtdrev),
					
					
					mtdtargetper: oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrevtarget),parseFloat(QObj.lastyearQ[key][0].mtdrevtarget)),
					mtdtargetperthird: mtdGRpre(QObj.currentyearQ[key][0].mtdrevtarget,QObj.thirdyearQ[key][0].mtdrevtarget),
					
					
					mtdopdper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdopd),parseFloat(QObj.lastyearQ[key][0].mtdopd)),
					mtdopdperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdopd/QObj.thirdyearQ[key][0].mtdopd),
					
					mtdopdyppper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdopdypp),parseFloat(QObj.lastyearQ[key][0].mtdopdypp)),
					mtdopdyppperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdopdypp,QObj.thirdyearQ[key][0].mtdopdypp),

					mtdctlowendper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdctlowend),parseFloat(QObj.lastyearQ[key][0].mtdctlowend)),
					mtdctlowendperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdctlowend,QObj.thirdyearQ[key][0].mtdctlowend),
					
					
					mtdcthighendper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdcthighend),parseFloat(QObj.lastyearQ[key][0].mtdcthighend)),
					mtdcthighendperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdcthighend,QObj.thirdyearQ[key][0].mtdcthighend),
					
					
					mtdctmidendper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdctmidend),parseFloat(QObj.lastyearQ[key][0].mtdctmidend)),
					mtdctmidendperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdctmidend,QObj.thirdyearQ[key][0].mtdctmidend),
					
					
					mtdctlowendcountper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdctlowendcount),parseFloat(QObj.lastyearQ[key][0].mtdctlowendcount)),
					mtdctlowendcountperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdctlowendcount,QObj.thirdyearQ[key][0].mtdctlowendcount),
					
					
					mtdcthighendcountper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdcthighendcount),parseFloat(QObj.lastyearQ[key][0].mtdcthighendcount)),
					mtdcthighendcountperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdcthighendcount,QObj.thirdyearQ[key][0].mtdcthighendcount),
					
					
					mtdctmidendcountper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdctmidendcount),parseFloat(QObj.lastyearQ[key][0].mtdctmidendcount)),
					mtdctmidendcountperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdctmidendcount,QObj.thirdyearQ[key][0].mtdctmidendcount),
					
					mtdallcatper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdallcat),parseFloat(QObj.lastyearQ[key][0].mtdallcat)),
					mtdallcatperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdallcat,QObj.thirdyearQ[key][0].mtdallcat),
					
					
					
					mtdallcatcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdallcatcount),parseFloat(QObj.lastyearQ[key][0].mtdallcatcount)),
					mtdallcatcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdallcatcount,QObj.thirdyearQ[key][0].mtdallcatcount),
					
					mtdoptper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdopt),parseFloat(QObj.lastyearQ[key][0].mtdopt)),
					mtdoptperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdopt,QObj.thirdyearQ[key][0].mtdopt),
					
					
					mtdoptcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdoptcount),parseFloat(QObj.lastyearQ[key][0].mtdoptcount)),
					mtdoptcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdoptcount,QObj.thirdyearQ[key][0].mtdoptcount),
					
					
					mtdoptyppper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdoptypp),parseFloat(QObj.lastyearQ[key][0].mtdoptypp)),
					mtdoptyppperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdoptypp,QObj.thirdyearQ[key][0].mtdoptypp),
					
					mtdphaper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdpha),parseFloat(QObj.lastyearQ[key][0].mtdpha)),
					mtdphaperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdpha,QObj.thirdyearQ[key][0].mtdpha),
					
					
					mtdphacountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdphacount),parseFloat(QObj.lastyearQ[key][0].mtdphacount)),
					mtdphacountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdphacount,QObj.thirdyearQ[key][0].mtdphacount),
					
					
					mtdphayppper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdphaypp),parseFloat(QObj.lastyearQ[key][0].mtdphaypp)),
					mtdphayppperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdphaypp,QObj.thirdyearQ[key][0].mtdphaypp),
					
					mtdtreatinvper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdtreatinv),parseFloat(QObj.lastyearQ[key][0].mtdtreatinv)),
					mtdtreatinvperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdtreatinv,QObj.thirdyearQ[key][0].mtdtreatinv),
					
					
					
					mtdtreatinvcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdtreatinvcount),parseFloat(QObj.lastyearQ[key][0].mtdtreatinvcount)),
					mtdtreatinvcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdtreatinvcount,QObj.thirdyearQ[key][0].mtdtreatinvcount),
					
					
					mtdcorper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdcor),parseFloat(QObj.lastyearQ[key][0].mtdcor)),
					mtdcorperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdcor,QObj.thirdyearQ[key][0].mtdcor),
					
					
					mtdcorcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdcorcount),parseFloat(QObj.lastyearQ[key][0].mtdcorcount)),
					mtdcorcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdcorcount,QObj.thirdyearQ[key][0].mtdcorcount),
					
					mtdrefper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdref),parseFloat(QObj.lastyearQ[key][0].mtdref)),
					mtdrefperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdref,QObj.thirdyearQ[key][0].mtdref),
					
					mtdrefcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrefcount),parseFloat(QObj.lastyearQ[key][0].mtdrefcount)),
					mtdrefcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdrefcount,QObj.thirdyearQ[key][0].mtdrefcount),
					
					mtdosuper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdosu),parseFloat(QObj.lastyearQ[key][0].mtdosu)),
					mtdosuperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdosu,QObj.thirdyearQ[key][0].mtdosu),
					
					
					mtdosucountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdosucount),parseFloat(QObj.lastyearQ[key][0].mtdosucount)),
					mtdosucountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdosucount,QObj.thirdyearQ[key][0].mtdosucount),
					
					mtdconlensper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdconlens),parseFloat(QObj.lastyearQ[key][0].mtdconlens)),
					mtdconlensperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdconlens,QObj.thirdyearQ[key][0].mtdconlens),
					
					
					
					mtdconlenscountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdconlenscount),parseFloat(QObj.lastyearQ[key][0].mtdconlenscount)),
					mtdconlenscountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdconlenscount,QObj.thirdyearQ[key][0].mtdconlenscount),
					
					mtdvrinper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdvrin),parseFloat(QObj.lastyearQ[key][0].mtdvrin)),
					mtdvrinperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdvrin,QObj.thirdyearQ[key][0].mtdvrin),
					
					mtdvrincountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdvrincount),parseFloat(QObj.lastyearQ[key][0].mtdvrincount)),
					mtdvrincountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdvrincount,QObj.thirdyearQ[key][0].mtdvrincount),
					
					mtdtconsuper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdtconsu),parseFloat(QObj.lastyearQ[key][0].mtdtconsu)),
					mtdtconsuperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdtconsu,QObj.thirdyearQ[key][0].mtdtconsu),
					
					
					mtdpopdper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdpopd),parseFloat(QObj.lastyearQ[key][0].mtdpopd)),
					mtdpopdperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdpopd,QObj.thirdyearQ[key][0].mtdpopd),
					
					mtdvrsugper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdvrsug),parseFloat(QObj.lastyearQ[key][0].mtdvrsug)),
					mtdvrsugperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdvrsug,QObj.thirdyearQ[key][0].mtdvrsug),
					
					
					mtdvrsugcountper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdvrsugcount),parseFloat(QObj.lastyearQ[key][0].mtdvrsugcount)),
					mtdvrsugcountperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdvrsugcount,QObj.thirdyearQ[key][0].mtdvrsugcount)
...
exports.insertOPRData = async (
	...
	let oprDataset = await OPRData(paid_op,total_consu,all_cat_surg,cat_low_end,cat_mid_end,cat_high_end,corena,referctive,							vr_inj,vr_surg,phar,optical,contactlens,other_sug,treat,invest,branches,year,month);
	return oprDataset;
	
}


let OPRData = async (paid_op,total_consu,all_cat_surg,cat_low_end,cat_mid_end,cat_high_end,corena,referctive,vr_inj,						vr_surg,phar,optical,contactlens,other_sug,treat,invest,branches,year,month)=> {
	...
	let obtDataObj = {};
	let branchWiseObj=[];
	let paid_op_count=0, totl_consu_charges=0,cat_surg_no=0,cat_surg_value=0,cat_low_end_no=0,cat_low_end_value=0,cat_mid_end_no=0,cat_mid_end_value=0,cat_high_end_no=0,cat_high_end_value=0,corneal_no=0,corneal_value=0,ref_no=0,ref_value=0,vr_surg_no=0,vr_surg_value=0,vr_inje_no=0,vr_inje_value=0,opt_num_orders=0,opt_value=0,optical_ypp=0,pha_no_bills=0,pha_value=0,pha_ypp=0,con_len_value=0,other_sug_no=0,other_sug_value=0,inv_no=0,inv_value=0,treat_no=0,treat_value=0,entityCode='',region='';
	
	branches.forEach(element => {
		paid_op_count=0, totl_consu_charges=0,cat_surg_no=0,cat_surg_value=0,cat_low_end_no=0,cat_low_end_value=0,cat_mid_end_no=0,cat_mid_end_value=0,cat_high_end_no=0,cat_high_end_value=0,corneal_no=0,corneal_value=0,ref_no=0,ref_value=0,vr_surg_no=0,vr_surg_value=0,vr_inje_no=0,vr_inje_value=0,opt_num_orders=0,opt_value=0,optical_ypp=0,pha_no_bills=0,pha_value=0,pha_ypp=0,con_len_value=0,other_sug_no=0,other_sug_value=0,inv_no=0,inv_value=0,treat_no=0,treat_value=0,entityCode='',region='';
		
		if(element.code=='VSH'){
			entityCode = 'AHI';
		}else{
			entityCode = element.entity;
		}
		...
		if(paid_op.length>0) {
		   ...
		   _.filter(paid_op, {   'NATIVE': element.code}).forEach(
						element => {
							paid_op_count= element.paid_op_ct;
			});
		}
		if(total_consu.length>0) {
			_.filter(total_consu, {   'NATIVE': element.code}).forEach(
						element => {
							totl_consu_charges = element.total_con_val;
			});
		}
		if(all_cat_surg.length>0) {
			_.filter(all_cat_surg, {   'NATIVE': element.code}).forEach(
						element => {							
							cat_surg_no= element.all_cat_surg_ct;
							cat_surg_value	= element.all_cat_surg_val;
			});
		}
		
		if(cat_low_end.length>0) {
			
			_.filter(cat_low_end, {   'NATIVE': element.code}).forEach(
						element => {
							cat_low_end_no= element.cat_low_end_ct;
							cat_low_end_value	= element.cat_low_end_val;
			});
		}
		if(cat_mid_end.length>0) {
			
			_.filter(cat_mid_end, {   'NATIVE': element.code}).forEach(
						element => {
							cat_mid_end_no= element.cat_mid_end_ct;
							cat_mid_end_value	= element.cat_mid_end_val;
			});
		}
		if(cat_high_end.length>0) {
			
			_.filter(cat_high_end, {   'NATIVE': element.code}).forEach(
						element => {
							cat_high_end_no= element.cat_high_end_ct;
							cat_high_end_value	= element.cat_high_end_val;
			});
		}
		if(corena.length>0) {
			
			_.filter(corena, {   'NATIVE': element.code}).forEach(
						element => {
							corneal_no= element.cornea_ct;
							corneal_value	= element.cornea_val;
			});
		}
		if(referctive.length>0) {
			
			_.filter(referctive, {   'NATIVE': element.code}).forEach(
						element => {
							ref_no= element.ref_ct;
							ref_value	= element.ref_val;
			});
		}
		if(vr_inj.length>0) {
			
			_.filter(vr_inj, {   'NATIVE': element.code}).forEach(
						element => {
							vr_inje_no= element.vr_inj_ct;
							vr_inje_value	= element.vr_inj_val;
			});
		}
		if(vr_surg.length>0) {
			
			_.filter(vr_surg, {   'NATIVE': element.code}).forEach(
						element => {
							vr_surg_no= element.vr_sug_ct;
							vr_surg_value	= element.vr_sug_val;
			});
		}
		if(phar.length>0) {
			_.filter(phar, {   'NATIVE': element.code}).forEach(
						element => {
							pha_no_bills= element.phar_ct;
							pha_value	= element.phar_val;
							pha_ypp     = (lakshFormatRevenue1(pha_value)/pha_no_bills)*100000;
			});
		}
		if(optical.length>0) {
			_.filter(optical, {   'NATIVE': element.code}).forEach(
						element => {
							opt_num_orders= element.optical_ct;
							opt_value	= element.optical_val;
							optical_ypp     = (lakshFormatRevenue1(opt_value)/opt_num_orders)*100000;
			});
		}
		if(contactlens.length>0) {
			_.filter(contactlens, {   'NATIVE': element.code}).forEach(
						element => {
							
							con_len_value	= element.contact_val;
							
			});
		}
		if(other_sug.length>0) {
			_.filter(other_sug, {   'NATIVE': element.code}).forEach(
						element => {
							other_sug_no= element.other_sug_ct;
							other_sug_value	= element.other_sug_val;
							
			});
		}
		if(treat.length>0) {
			_.filter(treat, {   'NATIVE': element.code}).forEach(
						element => {
							treat_no= element.treat_ct;
							treat_value	= element.treat_val;
							
			});
		}
		if(invest.length>0) {
			_.filter(invest, {   'NATIVE': element.code}).forEach(
						element => {
							inv_no= element.invest_ct;
							inv_value	= element.invest_val;
			 ...
			 branchWiseObj.push({
					...
					'year' : year,
					'month' : month,					
					'paid_review': paid_op_count,
					'totl_consu_charges': totl_consu_charges,
					'ypp_new_op':0,
					'cat_surg_no': cat_surg_no,
					'cat_surg_value': cat_surg_value,
					'cat_low_end_no': cat_low_end_no,
					'cat_low_end_value': cat_low_end_value,
					'cat_mid_end_no': cat_mid_end_no,
					'cat_mid_end_value': cat_mid_end_value,
					'cat_high_end_no': cat_high_end_no,
					'cat_high_end_value': cat_high_end_value,
					'corneal_no': corneal_no,
					'corneal_value': corneal_value,
					'ref_no': ref_no,
					'ref_value': ref_value,
					'vr_surg_no': vr_surg_no,
					'vr_surg_value': vr_surg_value,
					'vr_inje_no': vr_inje_no,
					'vr_inje_value': vr_inje_value,
					'pha_no_bills': pha_no_bills,
					'pha_value': pha_value,
					'pha_ypp': pha_ypp,
					'opt_num_orders': opt_num_orders,
					'opt_value': opt_value,
					'optical_ypp': optical_ypp,
					'con_len_value': con_len_value,
					'other_sug_no': other_sug_no,
					'other_sug_value': other_sug_value,
					'inv_no': inv_no,
					'inv_value': inv_value,
					'treat_no': treat_no,
	...
	obtDataObj['OPRData'] = branchWiseObj;
...
exports.snapshotREbita = async ( 
  ...
  opr_ebita_res
	...
	let reveCalMonthRes = await reveCalMonthWiseEbita( rev_det_res,rev_target_res,threeyearsbef,twoyearsbef,currentFinYear,currentFinYearTo,opr_ebita_res);
	let reveCalQRes = await reveCalQWiseEbita(reveCalMonthRes);
...
let reveCalMonthWiseEbita = async ( rev_det_res,rev_target_res,threeyearsbef,twoyearsbef,currentFinYear,currentFinYearTo,opr_ebita_res) => {
	//console.log(opr_terget_query_res);
	let threeYearBeforevFinYearArr = [parseInt(threeyearsbef),parseInt(twoyearsbef)];
	let prevFinYearArr = [parseInt(twoyearsbef),parseInt(currentFinYear)];
	let currentFinYearArr = [parseInt(currentFinYear),parseInt(currentFinYearTo)];	
	let mtdrev = 0 ,monthNumber ='',monthText ='',mtdrevtarget=0,totalrev=0,totaltarget=0;
	let mtdrevwotax=0,totalrevwotax=0,mtdrevwotaxtarget=0,totalrevwotaxtarget=0;
	let mtdmc=0,totalmc=0,mtdmctarget=0,totalmctarget=0;	
	let mtdgrmargin=0,totalgrmargin=0,mtdgrmargintarget=0,totalgrmargintarget=0;	
	let mtddrcost=0,totaldrcost=0,mtddrcosttarget=0,totaldrcosttarget=0;
	let mtdstaffcost=0,totalstaffcost=0,mtdstaffcosttarget=0,totalstaffcosttarget=0;
	let mtdadmincost=0,totaladmincost=0,mtdadmincosttarget=0,totaladmincosttarget=0;
	let mtdmarket=0,totalmarket=0,mtdmarkettarget=0,totalmarkettarget=0;
	let mtdrent=0,totalrent=0,mtdrenttarget=0,totalrenttarget=0;
	let mtdrenteq=0,totalrenteq=0,mtdrenteqtarget=0,totalrenteqtarget=0;
	let mtdbrnlelebit=0,totalbrnlelebit=0,mtdbrnlelebittarget=0,totalbrnlelebittarget=0;
	let mtdregsalcost=0,totalregsalcost=0,mtdregsalcosttarget=0,totalregsalcosttarget=0;
	let mtdregadmincost=0,totalregadmincost=0,mtdregadmincosttarget=0,totalregadmincosttarget=0;
	let mtdregmarket=0,totalregmarket=0,mtdregmarkettarget=0,totalregmarkettarget=0;
	let mtdreglelebit=0,totalreglelebit=0,mtdreglelebittarget=0,totalreglelebittarget=0;
	let mtdcorlelebit=0,totalcorlelebit=0,mtdcorlelebittarget=0,totalcorlelebittarget=0;
   ...
   curFinYearMonthSet[currentFinYearTo] = [1,2,3];
	...
	prevFinYearArr.forEach(year => {		     
			 prevFinYearMonthSet[year].forEach(month => {
				 mtdrevtarget=0;
				 mtdrev = 0;
				 mtdrevwotax=0,mtdrevwotaxtarget=0;				 
				 mtdmc=0,mtdmctarget=0;
				 mtdgrmargin=0,mtdgrmargintarget=0;
				 mtddrcost=0,mtddrcosttarget=0;
				 mtdstaffcost=0,mtdstaffcosttarget=0;
				 mtdadmincost=0,mtdadmincosttarget=0;
				 mtdmarket=0,mtdmarkettarget=0;
				 mtdrent=0,mtdrenttarget=0;
				 mtdrenteq=0,mtdrenteqtarget=0;
				 mtdbrnlelebit=0,mtdbrnlelebittarget=0;
				 mtdregsalcost=0,mtdregsalcosttarget=0;
				 mtdregadmincost=0,mtdregadmincosttarget=0;
				 mtdregmarket=0,mtdregmarkettarget=0;
				 mtdreglelebit=0,mtdreglelebittarget=0;
				 mtdcorlelebit=0,mtdcorlelebittarget=0;				 
				 monthText = monthMapping[month];				 
				_.filter(rev_det_res, {  'MONTH(trans_date)': month,'yr' : year}).forEach(
					element => {
						mtdrev+= element.mtd;
				...
				*/
				...
				_.filter(opr_ebita_res, {   'month': month,'year' : year}).forEach(
					element => {
						mtdrevwotaxtarget+= element.total_revenue_target;
						mtdrevwotax+= element.total_revenue;
						mtdmctarget+= parseFloat(element.cogs_target)+parseFloat(element.ref_fee_target);
						mtdmc+=  parseFloat(element.cogs)+parseFloat(element.ref_fee);
						mtdgrmargin+= parseFloat(mtdrevwotax)-parseFloat(mtdmc);
						mtdgrmargintarget+= parseFloat(mtdrevwotaxtarget)-parseFloat(mtdmctarget);
						mtddrcost+= element.doctor_val;
						mtddrcosttarget+= element.doctor_target;
						mtdstaffcost+= element.staff_val;
						mtdstaffcosttarget+= element.staff_target;
						mtdadmincost+= element.admin_val;
						mtdadmincosttarget+= element.admin_target;
						mtdmarket+= element.marketing_val;
						mtdmarkettarget+= element.marketing_target;
						mtdrent+= element.rent_val;
						mtdrenttarget+= element.rent_target;
						mtdrenteq+= element.rent_eq_val;
						mtdrenteqtarget+= element.rent_eq_target;						
						mtdbrnlelebit+= element.brn_level_eb_val;
						mtdregsalcost+= element.reg_salary_val;
						mtdregadmincost+= element.reg_admin_val;
						mtdregmarket+= element.reg_market_val;
						mtdreglelebit+= element.reg_level_eb_val;
						mtdcorlelebit+= element.corp_level_eb_val;						
						mtdbrnlelebittarget+= element.brn_level_eb_target;
						mtdregsalcosttarget+= element.reg_salary_target;
						mtdregadmincosttarget+= element.reg_admin_target;
						mtdregmarkettarget+= element.reg_market_target;
						mtdreglelebittarget+= element.reg_level_eb_target;
						mtdcorlelebittarget+= element.corp_level_eb_target;
				});
				
				totalrevwotaxtarget+= parseFloat(mtdrevwotaxtarget);
				totalrevwotax+= parseFloat(mtdrevwotax);
				totalmctarget+= parseFloat(mtdmctarget);
				totalmc+= parseFloat(mtdmc);
				totalgrmargin+= parseFloat(mtdgrmargin);
				totalgrmargintarget+= parseFloat(mtdgrmargintarget);
				totaldrcost+= parseFloat(mtddrcost);
				totaldrcosttarget+= parseFloat(mtddrcosttarget);
				totalstaffcost+= parseFloat(mtdstaffcost);
				totalstaffcosttarget+= parseFloat(mtdstaffcosttarget);
				totaladmincost+= parseFloat(mtdadmincost);
				totaladmincosttarget+= parseFloat(mtdadmincosttarget);
				totalmarket+= parseFloat(mtdmarket);
				totalmarkettarget+= parseFloat(mtdmarkettarget);
				totalrent+= parseFloat(mtdrent);
				totalrenttarget+= parseFloat(mtdrenttarget);
				totalrenteq+= parseFloat(mtdrenteq);
				totalrenteqtarget+= parseFloat(mtdrenteqtarget);				
				totalbrnlelebit+= parseFloat(mtdbrnlelebit);
				totalbrnlelebittarget+= parseFloat(mtdbrnlelebittarget);
				totalregsalcost+= parseFloat(mtdregsalcost);
				totalregsalcosttarget+= parseFloat(mtdregsalcosttarget);
				totalregadmincost+= parseFloat(mtdregadmincost);
				totalregadmincosttarget+= parseFloat(mtdregadmincosttarget);
				totalregmarket+= parseFloat(mtdregmarket);
				totalregmarkettarget+= parseFloat(mtdregmarkettarget);
				totalreglelebit+= parseFloat(mtdreglelebit);
				totalreglelebittarget+= parseFloat(mtdreglelebittarget);
				totalcorlelebit+= parseFloat(mtdcorlelebit);
				totalcorlelebittarget+= parseFloat(mtdcorlelebittarget);
				 ...
				 monthWiseObj.push({			
					...
					mtdrev: lakshFormatRevenue1(mtdrev),
					mtdrevtarget: mtdrevtarget.toFixed(1),
					mtdrevwotaxtarget:mtdrevwotaxtarget.toFixed(1),
					mtdrevwotax:mtdrevwotax.toFixed(1),
					mtdmctarget:mtdmctarget.toFixed(1),
					mtdmc:mtdmc.toFixed(1),
					mtdmcper : oprPer(mtdmc,mtdrevwotax),
					mtdmcpertarget : oprPer(mtdmctarget,mtdrevwotaxtarget),
					mtdgrmargin:mtdgrmargin.toFixed(1),
					mtdgrmargintarget:mtdgrmargintarget.toFixed(1),
					mtddrcost:mtddrcost.toFixed(1),
					mtddrcosttarget:mtddrcosttarget.toFixed(1),
					mtdstaffcost:mtdstaffcost.toFixed(1),
					mtdstaffcosttarget:mtdstaffcosttarget.toFixed(1),
					mtdadmincost:mtdadmincost.toFixed(1),
					mtdadmincosttarget:mtdadmincosttarget.toFixed(1),
					mtdmarket:mtdmarket.toFixed(1),
					mtdmarkettarget:mtdmarkettarget.toFixed(1),
					mtdrent:mtdrent.toFixed(1),
					mtdrenttarget:mtdrenttarget.toFixed(1),
					mtdrenteq:mtdrenteq.toFixed(1),
					mtdrenteqtarget:mtdrenteqtarget.toFixed(1),
					mtdbrnlelebit:mtdbrnlelebit.toFixed(1),
					mtdbrnlelebittarget:mtdbrnlelebittarget.toFixed(1),
					mtdregsalcost:mtdregsalcost.toFixed(1),
					mtdregsalcosttarget:mtdregsalcosttarget.toFixed(1),
					mtdregadmincost:mtdregadmincost.toFixed(1),
					mtdregadmincosttarget:mtdregadmincosttarget.toFixed(1),
					mtdregmarket:mtdregmarket.toFixed(1),
					mtdregmarkettarget:mtdregmarkettarget.toFixed(1),					
					mtdreglelebit:mtdreglelebit.toFixed(1),
					mtdreglelebittarget:mtdreglelebittarget.toFixed(1),
					mtdcorlelebit:mtdcorlelebit.toFixed(1),
					mtdcorlelebittarget:mtdcorlelebittarget.toFixed(1)
				});	
	...
	branchObj['lastyeartotal'] = {
					totalrev: lakshFormatRevenue1(totalrev),
					totaltarget: totaltarget.toFixed(1),
					totalrevwotaxtarget: totalrevwotaxtarget.toFixed(1),
					totalrevwotax: totalrevwotax.toFixed(1),
					totalmctarget: totalmctarget.toFixed(1),
					totalmc: totalmc.toFixed(1),
					totalmcper : oprPer(totalmc,totalrevwotax),
					totalmcpertarget : oprPer(totalmctarget,totalrevwotaxtarget),
					totalgrmargin: totalgrmargin.toFixed(1),
					totalgrmargintarget: totalgrmargintarget.toFixed(1),
					totaldrcost: totaldrcost.toFixed(1),					
					totaldrcosttarget: totaldrcosttarget.toFixed(1),
					totalstaffcost: totalstaffcost.toFixed(1),
					totalstaffcosttarget: totalstaffcosttarget.toFixed(1),
					totaladmincost: totaladmincost.toFixed(1),
					totaladmincosttarget: totaladmincosttarget.toFixed(1),
					totalmarket: totalmarket.toFixed(1),
					totalmarkettarget: totalmarkettarget.toFixed(1),
					totalrent: totalrent.toFixed(1),					
					totalrenttarget: totalrenttarget.toFixed(1),
					totalrenteq: totalrenteq.toFixed(1),
					totalrenteqtarget: totalrenteqtarget.toFixed(1),
					totalbrnlelebit: totalbrnlelebit.toFixed(1),
					totalbrnlelebittarget: totalbrnlelebittarget.toFixed(1),
					totalregsalcost: totalregsalcost.toFixed(1),
					totalregsalcosttarget: totalregsalcosttarget.toFixed(1),
					totalregadmincost: totalregadmincost.toFixed(1),					
					totalregadmincosttarget: totalregadmincosttarget.toFixed(1),
					totalregmarket: totalregmarket.toFixed(1),
					totalregmarkettarget: totalregmarkettarget.toFixed(1),
					totalreglelebit: totalreglelebit.toFixed(1),
					totalreglelebittarget: totalreglelebittarget.toFixed(1),
					totalcorlelebit: totalcorlelebit.toFixed(1),
					totalcorlelebittarget: totalcorlelebittarget.toFixed(1)
					};
	monthWiseObj=[];
	mtdrevtarget=0;
	mtdrev = 0;	
	totalrev=0;
	totaltarget=0;
	mtdrevwotax=0,totalrevwotax=0,mtdrevwotaxtarget=0,totalrevwotaxtarget=0;
	mtdmc=0,totalmc=0,mtdmctarget=0,totalmctarget=0;	
	mtdgrmargin=0,totalgrmargin=0,mtdgrmargintarget=0,totalgrmargintarget=0;	
	mtddrcost=0,totaldrcost=0,mtddrcosttarget=0,totaldrcosttarget=0;
	mtdstaffcost=0,totalstaffcost=0,mtdstaffcosttarget=0,totalstaffcosttarget=0;
	mtdadmincost=0,totaladmincost=0,mtdadmincosttarget=0,totaladmincosttarget=0;
	mtdmarket=0,totalmarket=0,mtdmarkettarget=0,totalmarkettarget=0;
	mtdrent=0,totalrent=0,mtdrenttarget=0,totalrenttarget=0;
	mtdrenteq=0,totalrenteq=0,mtdrenteqtarget=0,totalrenteqtarget=0;
	 mtdbrnlelebit=0,totalbrnlelebit=0,mtdbrnlelebittarget=0,totalbrnlelebittarget=0;
	 mtdregsalcost=0,totalregsalcost=0,mtdregsalcosttarget=0,totalregsalcosttarget=0;
	 mtdregadmincost=0,totalregadmincost=0,mtdregadmincosttarget=0,totalregadmincosttarget=0;
	 mtdregmarket=0,totalregmarket=0,mtdregmarkettarget=0,totalregmarkettarget=0;
	 mtdreglelebit=0,totalreglelebit=0,mtdreglelebittarget=0,totalreglelebittarget=0;
	 mtdcorlelebit=0,totalcorlelebit=0,mtdcorlelebittarget=0,totalcorlelebittarget=0;
	...
	branchObj['totalper'] = { mtdrevper : oprPer(parseFloat(branchObj.currentyeartotal.totalrev),parseFloat(branchObj.lastyeartotal.totalrev)),
							mtdrevperthird : mtdGRpre(branchObj.currentyeartotal.totalrev,branchObj.thirdyeartotal.totalrev),
							revwotaxper : oprPer(parseFloat(branchObj.currentyeartotal.totalrevwotax),parseFloat(branchObj.lastyeartotal.totalrevwotax)),
							revwotaxperthird : mtdGRpre(branchObj.currentyeartotal.totalrevwotax,branchObj.thirdyeartotal.totalrevwotax),
							mcper : oprPer(parseFloat(branchObj.currentyeartotal.totalmc),parseFloat(branchObj.lastyeartotal.totalmc)),
							mcperthird : mtdGRpre(branchObj.currentyeartotal.totalmc,branchObj.thirdyeartotal.totalmc),
							grmarginper : oprPer(parseFloat(branchObj.currentyeartotal.totalgrmargin),parseFloat(branchObj.lastyeartotal.totalgrmargin)),
							
							mcper1 : oprPer(parseFloat(branchObj.currentyeartotal.totalmcper),parseFloat(branchObj.lastyeartotal.totalmcper)),
							mcper1third : mtdGRpre(branchObj.currentyeartotal.totalmcper,branchObj.thirdyeartotal.totalmcper),
							grmarginper : oprPer(parseFloat(branchObj.currentyeartotal.totalgrmargin),parseFloat(branchObj.lastyeartotal.totalgrmargin)),
							
							grmarginperthird : mtdGRpre(branchObj.currentyeartotal.totalgrmargin,branchObj.thirdyeartotal.totalgrmargin),						
							drcostper:oprPer(parseFloat(branchObj.currentyeartotal.totaldrcost),parseFloat(branchObj.lastyeartotal.totaldrcost)),
							drcostperthird:mtdGRpre(branchObj.currentyeartotal.totaldrcost,branchObj.thirdyeartotal.totaldrcost),							
							staffcostper : oprPer(parseFloat(branchObj.currentyeartotal.totalstaffcost),parseFloat(branchObj.lastyeartotal.totalstaffcost)),
							staffcostperthird : mtdGRpre(branchObj.currentyeartotal.totalstaffcost,branchObj.thirdyeartotal.totalstaffcost),
							admincostper : oprPer(parseFloat(branchObj.currentyeartotal.totaladmincost),parseFloat(branchObj.lastyeartotal.totaladmincost)),
							admincostperthird : mtdGRpre(branchObj.currentyeartotal.totaladmincost,branchObj.thirdyeartotal.totaladmincost),							
							marketper:oprPer(parseFloat(branchObj.currentyeartotal.totalmarket),parseFloat(branchObj.lastyeartotal.totalmarket)),
							marketperthird:mtdGRpre(branchObj.currentyeartotal.totalmarket,branchObj.thirdyeartotal.totalmarket),							
							rentper : oprPer(parseFloat(branchObj.currentyeartotal.totalrent),parseFloat(branchObj.lastyeartotal.totalrent)),
							rentperthird : mtdGRpre(branchObj.currentyeartotal.totalrent,branchObj.thirdyeartotal.totalrent),					
							renteqper:oprPer(parseFloat(branchObj.currentyeartotal.totalrenteq),parseFloat(branchObj.lastyeartotal.totalrenteq)),
							renteqperthird:mtdGRpre(branchObj.currentyeartotal.totalrenteq,branchObj.thirdyeartotal.totalrenteq),
							brnlelebitper : oprPer(parseFloat(branchObj.currentyeartotal.totalbrnlelebit),parseFloat(branchObj.lastyeartotal.totalbrnlelebit)),
							brnlelebitperthird : mtdGRpre(branchObj.currentyeartotal.totalbrnlelebit,branchObj.thirdyeartotal.totalbrnlelebit),
							regsalcostper : oprPer(parseFloat(branchObj.currentyeartotal.totalregsalcost),parseFloat(branchObj.lastyeartotal.totalregsalcost)),
							regsalcostperthird : mtdGRpre(branchObj.currentyeartotal.totalregsalcost,branchObj.thirdyeartotal.totalregsalcost),
							regadmincostper : oprPer(parseFloat(branchObj.currentyeartotal.totalregadmincost),parseFloat(branchObj.lastyeartotal.totalregadmincost)),
							regadmincostperthird : mtdGRpre(branchObj.currentyeartotal.totalregadmincost,branchObj.thirdyeartotal.totalregadmincost),				
							regmarketper : oprPer(parseFloat(branchObj.currentyeartotal.totalregmarket),parseFloat(branchObj.lastyeartotal.totalregmarket)),
							regmarketperthird : mtdGRpre(branchObj.currentyeartotal.totalregmarket,branchObj.thirdyeartotal.totalregmarket),							
							reglelebitper : oprPer(parseFloat(branchObj.currentyeartotal.totalreglelebit),parseFloat(branchObj.lastyeartotal.totalreglelebit)),
							reglelebitperthird : mtdGRpre(branchObj.currentyeartotal.totalreglelebit,branchObj.thirdyeartotal.totalreglelebit),							
							corlelebitper : oprPer(parseFloat(branchObj.currentyeartotal.totalcorlelebit),parseFloat(branchObj.lastyeartotal.totalcorlelebit)),
							corlelebitperthird : mtdGRpre(branchObj.currentyeartotal.totalcorlelebit,branchObj.thirdyeartotal.totalcorlelebit)
							}
	...
	branchObj.lastyearmonth.forEach(key => {   
		...
		monthWiseObj.push({			
					...
					mtdrevper : oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrev),parseFloat(key.mtdrev)),
					mtdrevperthird : mtdGRpre(branchObj.currentyearmonth[k].mtdrev,branchObj.thirdyearmonth[k].mtdrev),
					mtdrevwotaxper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrevwotax),parseFloat(key.mtdrevwotax)),
					mtdrevwotaxperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdrevwotax,branchObj.thirdyearmonth[k].mtdrevwotax),
					mtdmcper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdmc),parseFloat(key.mtdmc)),
					mtdmcperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdmc,branchObj.thirdyearmonth[k].mtdmc),
					mtdmcper1:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdmcper),parseFloat(key.mtdmcper)),
					mtdmcper1third:mtdGRpre(branchObj.currentyearmonth[k].mtdmcper,branchObj.thirdyearmonth[k].mtdmcper),
					
					
					mtdgrmarginper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdgrmargin),parseFloat(key.mtdgrmargin)),
					mtdgrmarginperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdgrmargin,branchObj.thirdyearmonth[k].mtdgrmargin),
					mtddrcostper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtddrcost),parseFloat(key.mtddrcost)),
					mtddrcostperthird:mtdGRpre(branchObj.currentyearmonth[k].mtddrcost,branchObj.thirdyearmonth[k].mtddrcost),
					mtdstaffcostper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdstaffcost),parseFloat(key.mtdstaffcost)),
					mtdstaffcostperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdstaffcost,branchObj.thirdyearmonth[k].mtdstaffcost),
					mtdadmincostper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdadmincost),parseFloat(key.mtdadmincost)),
					mtdadmincostperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdadmincost,branchObj.thirdyearmonth[k].mtdadmincost),
					mtdmarketper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdmarket),parseFloat(key.mtdmarket)),
					mtdmarketperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdmarket,branchObj.thirdyearmonth[k].mtdmarket),
					mtdrentper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrent),parseFloat(key.mtdrent)),
					mtdrentperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdrent,branchObj.thirdyearmonth[k].mtdrent),
					mtdrenteqper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdrenteq),parseFloat(key.mtdrenteq)),
					mtdrenteqperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdrenteq,branchObj.thirdyearmonth[k].mtdrenteq),
					mtdbrnlelebitper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdbrnlelebit),parseFloat(key.mtdbrnlelebit)),
					mtdbrnlelebitperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdbrnlelebit,branchObj.thirdyearmonth[k].mtdbrnlelebit),					
					mtdregsalcostper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdregsalcost),parseFloat(key.mtdregsalcost)),
					mtdregsalcostperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdregsalcost,branchObj.thirdyearmonth[k].mtdregsalcost),					
					mtdregadmincostper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdregadmincost),parseFloat(key.mtdregadmincost)),
					mtdregadmincostperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdregadmincost,branchObj.thirdyearmonth[k].mtdregadmincost),					
					mtdregmarketper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdregmarket),parseFloat(key.mtdregmarket)),
					mtdregmarketperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdregmarket,branchObj.thirdyearmonth[k].mtdregmarket),					
					mtdreglelebitper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdreglelebit),parseFloat(key.mtdreglelebit)),
					mtdreglelebitperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdreglelebit,branchObj.thirdyearmonth[k].mtdreglelebit),					
					mtdcorlelebitper:oprPer(parseFloat(branchObj.currentyearmonth[k].mtdcorlelebit),parseFloat(key.mtdcorlelebit)),
					mtdcorlelebitperthird:mtdGRpre(branchObj.currentyearmonth[k].mtdcorlelebit,branchObj.thirdyearmonth[k].mtdcorlelebit)
		});		
...
let reveCalQWiseEbita = async ( reveCalMonthRes) => {
	...
	let Qrevtar = 0;
	let QRevenueObj = {};	
	let Qgrmargin=0,Qgrmargintarget=0;
	let Qrevwotax=0,Qrevwotaxtarget=0;
	let Qmc=0,Qmctarget=0;
	let Qdrcost=0,Qdrcosttarget=0;
	let Qstaffcost=0,Qstaffcosttarget=0;
	let Qadmincost=0,Qadmincosttarget=0;
	let Qmarket=0,Qmarkettarget=0;
	let Qrent=0,Qrenttarget=0;
	let Qrenteq=0,Qrenteqtarget=0;
	let Qbrnlelebit=0,Qbrnlelebittarget=0;
	let	Qregsalcost=0,Qregsalcosttarget=0;
	let	Qregadmincost=0,Qregadmincosttarget=0;
	let	Qregmarket=0,Qregmarkettarget=0;
	let Qreglelebit=0,Qreglelebittarget=0;
	let Qcorlelebit=0,Qcorlelebittarget=0;
	QArr.forEach(Q => {
        Qrev=0;	Qrevtar=0;
		Qrevwotax=0,Qrevwotaxtarget=0;
		Qmc=0,Qmctarget=0;
		Qgrmargin=0,Qgrmargintarget=0;
		Qdrcost=0,Qdrcosttarget=0;
		Qstaffcost=0,Qstaffcosttarget=0;
		Qadmincost=0,Qadmincosttarget=0;
		Qmarket=0,Qmarkettarget=0;
		Qrent=0,Qrenttarget=0;
		Qrenteq=0,Qrenteqtarget=0;
		Qbrnlelebit=0,Qbrnlelebittarget=0;
		Qregsalcost=0,Qregsalcosttarget=0;
		Qregadmincost=0,Qregadmincosttarget=0;
		Qregmarket=0,Qregmarkettarget=0;
		Qreglelebit=0,Qreglelebittarget=0;
	    Qcorlelebit=0,Qcorlelebittarget=0;
		QRevenueObj[Q] = [];		
		QGroup[Q].forEach(month => {			
			_.filter(reveCalMonthRes.lastyearmonth, {  'monthText': month}).forEach(
						element => {
							Qrev+=parseFloat(element.mtdrev);							
							Qrevtar+=parseFloat(element.mtdrevtarget);
							Qrevwotax+=parseFloat(element.mtdrevwotax);							
							Qrevwotaxtarget+=parseFloat(element.mtdrevwotaxtarget);
							Qmc+=parseFloat(element.mtdmc);							
							Qmctarget+=parseFloat(element.mtdmctarget);
							Qgrmargin+=parseFloat(element.mtdgrmargin);							
							Qgrmargintarget+=parseFloat(element.mtdgrmargintarget);
							Qdrcost+=parseFloat(element.mtddrcost);							
							Qdrcosttarget+=parseFloat(element.mtddrcosttarget);
							Qstaffcost+=parseFloat(element.mtdstaffcost);							
							Qstaffcosttarget+=parseFloat(element.mtdstaffcosttarget);
							Qadmincost+=parseFloat(element.mtdadmincost);							
							Qadmincosttarget+=parseFloat(element.mtdadmincosttarget);
							Qmarket+=parseFloat(element.mtdmarket);							
							Qmarkettarget+=parseFloat(element.mtdmarkettarget);
							Qrent+=parseFloat(element.mtdrent);							
							Qrenttarget+=parseFloat(element.mtdrenttarget);
							Qrenteq+=parseFloat(element.mtdrenteq);							
							Qrenteqtarget+=parseFloat(element.mtdrenteqtarget);
							Qbrnlelebit+=parseFloat(element.mtdbrnlelebit);
							Qbrnlelebittarget+=parseFloat(element.mtdbrnlelebittarget);
							Qregsalcost+=parseFloat(element.mtdregsalcost);
							Qregsalcosttarget+=parseFloat(element.mtdregsalcosttarget);
							Qregadmincost+=parseFloat(element.mtdregadmincost);
							Qregadmincosttarget+=parseFloat(element.mtdregadmincosttarget);
							Qregmarket+=parseFloat(element.mtdregmarket);
							Qregmarkettarget+=parseFloat(element.mtdregmarkettarget);							
							Qreglelebit+=parseFloat(element.mtdreglelebit);
							Qreglelebittarget+=parseFloat(element.mtdreglelebittarget);							
							Qcorlelebit+=parseFloat(element.mtdcorlelebit);
							Qcorlelebittarget+=parseFloat(element.mtdcorlelebittarget);
		...
		QRevenueObj[Q].push({			
					mtdrev : Qrev.toFixed(1),
					mtdrevtarget: Qrevtar.toFixed(1),
					mtdrevwotax : Qrevwotax.toFixed(1),
					mtdrevwotaxtarget : Qrevwotaxtarget.toFixed(1),
					mtdmc : Qmc.toFixed(1),
					mtdmctarget : Qmctarget.toFixed(1),
					mtdmcper : oprPer(Qmc,Qrevwotax),
					mtdmcpertarget : oprPer(Qmctarget,Qrevwotaxtarget),
					mtdgrmargin : Qgrmargin.toFixed(1),
					mtdgrmargintarget : Qgrmargintarget.toFixed(1),
					mtddrcost : Qdrcost.toFixed(1),
					mtddrcosttarget : Qdrcosttarget.toFixed(1),
					mtdstaffcost : Qstaffcost.toFixed(1),
					mtdstaffcosttarget : Qstaffcosttarget.toFixed(1),
					mtdadmincost : Qadmincost.toFixed(1),
					mtdadmincosttarget : Qadmincosttarget.toFixed(1),
					mtdmarket : Qmarket.toFixed(1),
					mtdmarkettarget : Qmarkettarget.toFixed(1),
					mtdrent : Qmarket.toFixed(1),
					mtdrenttarget : Qrenttarget.toFixed(1),
					mtdrenteq : Qrenteq.toFixed(1),
					mtdrenteqtarget : Qrenteqtarget.toFixed(1),					
					mtdbrnlelebit : Qbrnlelebit.toFixed(1),
					mtdbrnlelebittarget : Qbrnlelebittarget.toFixed(1),
					mtdregsalcost : Qregsalcost.toFixed(1),
					mtdregsalcosttarget : Qregsalcosttarget.toFixed(1),
					mtdregadmincost : Qregadmincost.toFixed(1),
					mtdregadmincosttarget : Qregadmincosttarget.toFixed(1),
					mtdregmarket : Qregmarket.toFixed(1),
					mtdregmarkettarget : Qregmarkettarget.toFixed(1),
					mtdreglelebit : Qreglelebit.toFixed(1),
					mtdreglelebittarget : Qreglelebittarget.toFixed(1),
					mtdcorlelebit : Qcorlelebit.toFixed(1),
					mtdcorlelebittarget : Qcorlelebittarget.toFixed(1)
	...
	for (let key in QObj.lastyearQ) {
		...
		QRevenueObj3[key].push({			
					mtdrevper : oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrev),parseFloat(QObj.lastyearQ[key][0].mtdrev)),
					mtdrevperthird : mtdGRpre(QObj.currentyearQ[key][0].mtdrev,QObj.thirdyearQ[key][0].mtdrev),
                    mtdrevwotaxper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrevwotax),parseFloat(QObj.lastyearQ[key][0].mtdrevwotax)),
					mtdrevwotaxperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdrevwotax,QObj.thirdyearQ[key][0].mtdrevwotax),
					mtdmcper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdmc),parseFloat(QObj.lastyearQ[key][0].mtdmc)),
					mtdmcperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdmc,QObj.thirdyearQ[key][0].mtdmc),	
					
					mtdmcper1:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdmcper),parseFloat(QObj.lastyearQ[key][0].mtdmcper)),
					mtdmcper1third:mtdGRpre(QObj.currentyearQ[key][0].mtdmcper,QObj.thirdyearQ[key][0].mtdmcper),
					
					
					
					mtdgrmarginper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdgrmargin),parseFloat(QObj.lastyearQ[key][0].mtdgrmargin)),
					mtdgrmarginperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdgrmargin,QObj.thirdyearQ[key][0].mtdgrmargin),
					mtddrcostper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtddrcost),parseFloat(QObj.lastyearQ[key][0].mtddrcost)),
					mtddrcostperthird:mtdGRpre(QObj.currentyearQ[key][0].mtddrcost,QObj.thirdyearQ[key][0].mtddrcost),
					mtdstaffcostper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdstaffcost),parseFloat(QObj.lastyearQ[key][0].mtdstaffcost)),
					mtdstaffcostperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdstaffcost,QObj.thirdyearQ[key][0].mtdstaffcost),
					mtdadmincostper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdadmincost),parseFloat(QObj.lastyearQ[key][0].mtdadmincost)),
					mtdadmincostperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdadmincost,QObj.thirdyearQ[key][0].mtdadmincost),
					mtdmarketper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdmarket),parseFloat(QObj.lastyearQ[key][0].mtdmarket)),
					mtdmarketperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdmarket,QObj.thirdyearQ[key][0].mtdmarket),
					mtdrentper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrent),parseFloat(QObj.lastyearQ[key][0].mtdrent)),
					mtdrentperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdrent,QObj.thirdyearQ[key][0].mtdrent),
					mtdrenteqper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdrenteq),parseFloat(QObj.lastyearQ[key][0].mtdrenteq)),
					mtdrenteqperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdrenteq,QObj.thirdyearQ[key][0].mtdrenteq),
					mtdbrnlelebitper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdbrnlelebit),parseFloat(QObj.lastyearQ[key][0].mtdbrnlelebit)),
					mtdbrnlelebitperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdbrnlelebit,QObj.thirdyearQ[key][0].mtdbrnlelebit),
					mtdregsalcostper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdregsalcost),parseFloat(QObj.lastyearQ[key][0].mtdregsalcost)),
					mtdregsalcostperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdregsalcost,QObj.thirdyearQ[key][0].mtdregsalcost),
					mtdregadmincostper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdregadmincost),parseFloat(QObj.lastyearQ[key][0].mtdregadmincost)),
					mtdregadmincostperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdregadmincost,QObj.thirdyearQ[key][0].mtdregadmincost),
					mtdregmarketper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdregmarket),parseFloat(QObj.lastyearQ[key][0].mtdregmarket)),
					mtdregmarketperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdregmarket,QObj.thirdyearQ[key][0].mtdregmarket),
					mtdreglelebitper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdreglelebit),parseFloat(QObj.lastyearQ[key][0].mtdreglelebit)),
					mtdreglelebitperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdreglelebit,QObj.thirdyearQ[key][0].mtdreglelebit),
					mtdcorlelebitper:oprPer(parseFloat(QObj.currentyearQ[key][0].mtdcorlelebit),parseFloat(QObj.lastyearQ[key][0].mtdcorlelebit)),
					mtdcorlelebitperthird:mtdGRpre(QObj.currentyearQ[key][0].mtdcorlelebit,QObj.thirdyearQ[key][0].mtdcorlelebit)
...
exports.collreconFinEmailTemp = async (finalresult, today) => {

  let templateFin = await coll_recon_fin_email_template(finalresult, today);
  return templateFin;
}

let coll_recon_fin_email_template = async (finalresult, today) => {
	
	
	

var ideamedcashamount = new Number(finalresult.totalres[0].CASH_AMOUNT).toLocaleString("en-IN");
var ideamedcardamount = new Number(finalresult.totalres[0].CARD_AMOUNT).toLocaleString("en-IN");
var totcashdeposit = new Number(finalresult.totalres[0].CASH_DEPOSIT).toLocaleString("en-IN");
var totcarddeposit = new Number(finalresult.totalres[0].CARD_DEPOSIT).toLocaleString("en-IN");
var cashdiff = new Number((finalresult.totalres[0].CASH_AMOUNT-finalresult.totalres[0].CASH_DEPOSIT)-finalresult.totalres[0].CASH_ADMIN).toLocaleString("en-IN");
var carddiff = new Number((finalresult.totalres[0].CARD_AMOUNT-finalresult.totalres[0].CARD_DEPOSIT)-finalresult.totalres[0].CARD_ADMIN).toLocaleString("en-IN");

let   coll_re_fin_tem = '<html><body> <p>Hi User <br>Below are the status for each branch during cash reconciliation for the day.</p><br><table><tr><td width="20%" >Total IM cash collection (YTD) :</td><td width="20%">'+ ideamedcashamount +'</td><td width="20%"></td><td width="20%" >Total IM card collection (YTD) :</td><td width="20%">'+ ideamedcardamount +'</td><td width="20%"></td></tr><tr><td width="20%" >Total cash Deposit (YTD) :</td><td width="20%">'+ totcashdeposit +'</td><td width="20%"></td><td width="20%" >Total card Deposit (YTD) :</td><td width="20%">'+ totcarddeposit +'</td><td width="20%"></td></tr><tr><td width="20%" >Total cash Deposit Difference (YTD) :</td><td width="20%">'+cashdiff+'</td><td width="20%"></td><td width="20%" >Total card Deposit Difference (YTD) :</td><td width="20%">'+carddiff+'</td><td width="20%"></td></tr></table><br><table border="1" cellspacing="0"> <tr><th>Branch</th><th>Date</th><th>Cash</th><th>Branch Deposit</th><th>Finance Input</th><th>Cash Diff(YTD)</th> <th>Remarks (Cash diff)</th><th>Card</th><th>Branch Deposit</th><th>Finance Input</th><th>Card Diff(YTD)</th><th>Remarks (Card diff)</th><th>Remarks</th> <th>Finance Comments</th> </tr>';
  ...
  for(var i=0; i<finalresult.list.length; i++){
	  ...
	  var carddiff='';
	  var branclr='';
      if(finalresult.list[i].CASH_DIFF !=0){
			casdiff = 'bgcolor="#f0ae19"'; 
			branclr = 'bgcolor="#7083a9"'; 
	  }

	 if(finalresult.list[i].CARD_DIFF !=0){
		carddiff = 'bgcolor="#f9e699"'; 
		branclr = 'bgcolor="#7083a9"'; 
	 }

	  
     coll_re_fin_tem +='<tr><td  width="5%" '+branclr+'>'+ finalresult.list[i].BRANCH +'</td><td width="7%">'+ finalresult.list[i].PAYMENT_OR_REFUND_DATE +'</td><td width="5%">'+new Number(finalresult.list[i].CASH_AMOUNT).toLocaleString("en-IN")+'</td> <td width="5%">'+ new Number(finalresult.list[i].CASH_DEPOSIT).toLocaleString("en-IN")+'</td> <td width="5%">'+new Number(finalresult.list[i].CASH_ADMIN).toLocaleString("en-IN")+'</td> <td width="5%" '+casdiff+'>'+new Number(finalresult.list[i].CASH_DIFF).toLocaleString("en-IN")+'</td><td width="10%">'+finalresult.list[i].cash_remark+'</td><td width="5%">'+new Number(finalresult.list[i].CARD_AMOUNT).toLocaleString("en-IN")+'</td>  <td width="5%"> '+new Number(finalresult.list[i].CARD_DEPOSIT).toLocaleString("en-IN") +'</td><td width="5%">'+new Number(finalresult.list[i].CARD_ADMIN).toLocaleString("en-IN") +'</td> <td width="5%" '+carddiff+'>'+new Number(finalresult.list[i].CARD_DIFF).toLocaleString("en-IN") +'</td> <td width="5%">'+finalresult.list[i].card_remark+'</td><td width="10%">'+finalresult.list[i].CH_REMARKS+'</td><td width="10%">'+finalresult.list[i].FINANCE_REMARKS+'</td> </tr>';
   }


  coll_re_fin_tem += '</table><br><b>Note: This report is auto generated, please do not reply.</b> <br><p>For any corrections, please drop a mail to  <a href="mailto:helpdesk@dragarwal.com">helpdesk@dragarwal.com</a>. </p> <br><p>Regards,</p><p>Dr.Agarwal IT Team</p> </body></html>';

//console.log(collectiontemplate);

  return coll_re_fin_tem;
}


exports.collreconCHTemplate =  (_res,callback) => {
 ...
 coll_re_fin_tem+= '<html><body> <p>Hi Branch <br>There seems to be some mis-matching which is listed below during cash reconciliation. For further clarification, speak with Finance team.</p><br><table border="1" cellspacing="0"> <tr><th>Branch</th><th>Date</th><th>Cash</th><th>Branch Deposit</th><th>Finance Input</th><th>Cash Diff(YTD)</th> <th>Remarks (Cash diff)</th><th>Card</th><th>Branch Deposit</th><th>Finance Input</th><th>Card Diff(YTD)</th><th>Remarks (Card diff)</th><th>Remarks</th> <th>Finance Comments</th> </tr><tr><td  width="5%" '+branclr+'>'+ _res.BRANCH +'</td><td width="7%">'+ _res.PAYMENT_OR_REFUND_DATE +'</td><td width="5%">'+new Number(_res.CASH_AMOUNT).toLocaleString("en-IN")+'</td> <td width="5%">'+ new Number(_res.CASH_DEPOSIT).toLocaleString("en-IN")+'</td> <td width="5%">'+new Number(_res.CASH_ADMIN).toLocaleString("en-IN")+'</td> <td width="5%" '+casdiff+'>'+new Number(_res.CASH_DIFF).toLocaleString("en-IN")+'</td><td width="10%">'+_res.cash_remark+'</td><td width="5%">'+new Number(_res.CARD_AMOUNT).toLocaleString("en-IN")+'</td>  <td width="5%"> '+new Number(_res.CARD_DEPOSIT).toLocaleString("en-IN") +'</td><td width="5%">'+new Number(_res.CARD_ADMIN).toLocaleString("en-IN") +'</td> <td width="5%" '+carddiff+'>'+new Number(_res.CARD_DIFF).toLocaleString("en-IN") +'</td> <td width="5%">'+_res.card_remark+'</td><td width="10%">'+_res.CH_REMARKS+'</td><td width="10%">'+_res.FINANCE_REMARKS+'</td> </tr></table><br><b>Note: This report is auto generated, please do not reply.</b> <br><p>For any corrections, please drop a mail to  <a href="mailto:helpdesk@dragarwal.com">helpdesk@dragarwal.com</a>. </p> <br><p>Regards,</p><p>Dr.Agarwal IT Team</p> </body></html>';
 callback(null,coll_re_fin_tem);
...
exports.collreconFinEmailTempWeekly = async (finalresult,today) => {
	
	
	let templateFinweek = await coll_recon_fin_email_template_week(finalresult, today);
    return templateFinweek;
}


let coll_recon_fin_email_template_week = async (finalresult, today) => {	
	
let   coll_re_fin_tem = '<html><body> <p>Hi User <br>Below are the status for each branch during cash reconciliation for the day.</p><br><table border="1" cellspacing="0"> <tr><th>Branch</th><th>Cash</th><th>Branch Deposit</th><th>Finance Input</th><th>Cash Diff(YTD)</th> <th>Card</th><th>Branch Deposit</th><th>Finance Input</th><th>Card Diff(YTD)</th> </tr>';

  for(var i=0; i<finalresult.header.length; i++){
      var casdiff='';
	  var carddiff='';
	  var branclr='';
      if(finalresult.header[i].cash_diff !=0){
			casdiff = 'bgcolor="#f0ae19"'; 
			
	  }

	 if(finalresult.header[i].card_diff !=0){
		carddiff = 'bgcolor="#f9e699"'; 
		
	 }

	  
     coll_re_fin_tem +='<tr><td bgcolor="#f9e699"  width="5%" '+branclr+'>'+ finalresult.header[i].parent_branch +'</td><td width="5%">'+new Number(finalresult.header[i].cash_amount).toLocaleString("en-IN")+'</td> <td width="5%">'+ new Number(finalresult.header[i].cash_deposit).toLocaleString("en-IN")+'</td> <td width="5%">'+new Number(finalresult.header[i].cash_admin).toLocaleString("en-IN")+'</td> <td width="5%" '+casdiff+'>'+new Number(finalresult.header[i].cash_diff).toLocaleString("en-IN")+'</td><td width="5%">'+new Number(finalresult.header[i].card_amount).toLocaleString("en-IN")+'</td>  <td width="5%"> '+new Number(finalresult.header[i].card_deposit).toLocaleString("en-IN") +'</td><td width="5%">'+new Number(finalresult.header[i].card_admin).toLocaleString("en-IN") +'</td> <td width="5%" '+carddiff+'>'+new Number(finalresult.header[i].card_diff).toLocaleString("en-IN") +'</td>  </tr>';
   }
  ...
  for(var i=0; i<finalresult.list.length; i++){
     ...
     coll_re_fin_tem +='<tr><td  width="5%" '+branclr+'>'+ finalresult.list[i].branch +'</td><td width="5%">'+new Number(finalresult.list[i].cash_amount).toLocaleString("en-IN")+'</td> <td width="5%">'+ new Number(finalresult.list[i].cash_deposit).toLocaleString("en-IN")+'</td> <td width="5%">'+new Number(finalresult.list[i].cash_admin).toLocaleString("en-IN")+'</td> <td width="5%" '+casdiff+'>'+new Number(finalresult.list[i].cash_diff).toLocaleString("en-IN")+'</td><td width="5%">'+new Number(finalresult.list[i].card_amount).toLocaleString("en-IN")+'</td>  <td width="5%"> '+new Number(finalresult.list[i].card_deposit).toLocaleString("en-IN") +'</td><td width="5%">'+new Number(finalresult.list[i].card_admin).toLocaleString("en-IN") +'</td> <td width="5%" '+carddiff+'>'+new Number(finalresult.list[i].card_diff).toLocaleString("en-IN") +'</td>  </tr>';
   }
